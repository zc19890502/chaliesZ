	...
	<div class="container-fluid whole ">
		...
		<div class="row-fluid" style="background-color:#F0F0F0; margin-top:-20px; min-height: 400px;">
			...
			<div class="span8" style="margin-left:20%">
				  <c:choose>
					  <c:when test="${requestScope.uploaderr==1 }">照片未选择</c:when>
					  <c:when test="${requestScope.uploaderr==2 }">身份证未选择</c:when>
					  <c:when test="${requestScope.uploaderr==3 }">工作证明未选择</c:when>
					  <c:when test="${requestScope.uploaderr==4 }">公司审核意见未选择</c:when>
					  <c:when test="${requestScope.uploaderr==5 }">您现在不能报名</c:when>
					  <c:otherwise>上传文件格式错误</c:otherwise>
				  </c:choose>
				  <h4><a href="doCheckResult.action" style="color:red;">点击此处</a>重新报名！</h4>
	  		</div>
		</div>
	</div>
 	<div class="container-fluid whole">
