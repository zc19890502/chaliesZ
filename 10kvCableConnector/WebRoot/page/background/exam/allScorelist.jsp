	...
	<div class="container-fluid"
		...
		<div class="row-fluid"
			...
			<div class="span9"
				...
				<form class="form-inline" action="doQueryAllScoreByCondition.action"
					method="post">
					<fieldset>
				...
				<form id="fom" name="fom">
					<table class="table ">
                        ...
                        <th align="center">考试ID</th>  -->
								...
								<th align="center">冷缩(中)</th>
								<th align="center">冷缩(终)</th>
								<th align="center">热缩(中)</th>
								<th align="center">热缩(终)</th>
						...
						<tbody>
							<tr>
								<s:splitPage pageSize="10"
									page="doQueryAllScoreByCondition.action?realName=${requestScope.realName }&company=${requestScope.company }"
									data="${socreInfoList }">
									<tr>
                    ...
                    <td align="center">${splitData.examInfoId}</td>  --%>
										...
										<td align="center"><c:choose>
												<c:when test="${splitData.coldMidScore<80.0 }">未通过</c:when>
												<c:when test="${splitData.coldMidScore>=80.0 }">通过</c:when>
											</c:choose></td>
										<td align="center"><c:choose>
												<c:when test="${splitData.coldTemScore<80.0 }">未通过</c:when>
												<c:when test="${splitData.coldTemScore>=80.0 }">通过</c:when>
											</c:choose></td>
										<td align="center"><c:choose>
												<c:when test="${splitData.hotMidScore<80.0 }">未通过</c:when>
												<c:when test="${splitData.hotMidScore>=80.0 }">通过</c:when>
											</c:choose></td>
										<td align="center"><c:choose>
												<c:when test="${splitData.hotTemScore<80.0 }">未通过</c:when>
												<c:when test="${splitData.hotTemScore>=80.0 }">通过</c:when>
											</c:choose></td>
										<td align="center"><a
											href="doAllScoreTableInfoById.action?examInfoId=${splitData.examInfoId}&userInfoId=${splitData.userInfoId}">修改成绩
										</a>
										</td>
									</tr>
								</s:splitPage>
							...
							<tr>
								<td colspan="10" style="text-align:right; border-bottom:none;">
									<button type="button" class="btn "
										onclick="javascript:location.href='doDownloadAllScoreXls.action?fileName=xls/down/operator.xls'">下载操作成绩xls</button>
									<button type="button" class="btn "
										onclick="javascript:location.href='doMangeAllScore.action'">批量上传</button>
								</td>
							</tr>
							<c:if test="${!empty socreInfoList }">
								<tr>
									<td colspan="10" style="text-align:right; border-bottom:none;">
										<button type="button" class="btn "
											onclick="javascript:if(confirm('生成证书后,将无法对成绩进行管理!')){location.href='doGenerateCert.action'}">生成证书</button>
