	...
	<div class="container-fluid"
		...
		<div class="row-fluid"
			...
			<div class="span9"
				...
				<form class="form-inline" action="doQueryNumberByCondition.action"
					method="post">
					<fieldset>
						...
						<table style="margin-left:40px; text-align:right;border:1px; "
							...
							<tr>
								...
								<td>邮编:</td>
								<td><input type="text" class="input-small" placeholder="邮编"
									id="place" name="c_place" value="${requestScope.place }">
								</td>
								<td>工作单位:</td>
								<td><input type="text" class="input-small"
			...
			<div class="span9"
				style="margin-top:10px; margin-left:50px; min-height: 600px;">
				<form id="fom" name="fom">
					<table class="table ">
						...
						<tody> <s:splitPage pageSize="10"
							page="doNumberTableInfo.action?realName=${requestScope.realName }&place=${requestScope.place }&company=${requestScope.company }"
							data="${numberInfoList }">
							<tr>

								<td align="center">${splitData.userInfoId}</td>
								<td align="center">${splitData.realName}</td>
								<td align="center">${splitData.idnum}</td>
								<td align="center">${splitData.place}</td>
								<td align="center">${splitData.company}</td>
								<td align="center">${splitData.number}</td>
								<td align="center"><a
									href="doNumberTableInfoById.action?examInfoId=${splitData.examInfoId}&userInfoId=${splitData.userInfoId}">
										操作 </a></td>

							</tr>
						</s:splitPage> <!-- 				<tr>
                	<td colspan="9" style="text-align:right; border-bottom: none;">
                		<button type="button" class="btn "onclick="javascript:location.href='doDownloadNumberXls.action?fileName=xls/down/number.xls'">下载xls</button>
                    	<button type="button" class="btn " onclick="javascript:location.href='doMangeNumber.action'">批量上传</button>
                    </td>
                </tr> --> </tody>
					</table>
				</form>
