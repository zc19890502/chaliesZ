		...
		<div class="row-fluid"
			...
			<div class="span9"
				...
				<form class="form-inline"
					action="doQueryOperateFeeByCondition.action" method="post">
					<fieldset>
						<legend>操作考试缴费管理</legend>
						<!-- 这里是判断搜索条件是否为空 -->
						...
						<table style="margin-left:40px; text-align:right;border:1px; "
							...
							<tr>
								...
								<td>所属单位:</td>
								<td><input type="text" class="input-small"
									placeholder="所属单位" id="company" name="c_company"
					...
					<table class="table table-hover ">
						...
						<tbody>
							<s:splitPage pageSize="10"
								page="doQueryOperateFeeByCondition.action?realName=${requestScope.realName }&company=${requestScope.company }&skillLeval${requestScope.skillLevel }"
									...
									<c:otherwise>
										<tr>
											...
											<td align="center"><c:choose>
													<c:when test="${splitData.checkState==3 }">
														<a
															href='doQueryExammeeOperateFeeById.action?examineeId=${splitData.examineeInfoId }'>审核</a>

													</c:when>
													<c:when test="${splitData.checkState==5 }">
														<a
															href='doQueryExammeeOperateFeeById.action?examineeId=${splitData.examineeInfoId }'>查看</a>
													</c:when>
													<c:when test="${splitData.checkState==6 }">
														<a
															href='doQueryExammeeOperateFeeById.action?examineeId=${splitData.examineeInfoId }'>审核</a>
